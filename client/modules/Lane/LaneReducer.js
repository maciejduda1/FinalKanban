...
export default function lanes(state = initialState, action) {
  switch (action.type) {
    ...
    /*  if (action.targetLaneId) {
        console.log('Czy w to wpadamy2? ', action.targetId);
        newLane = { ...state[action.targetLaneId] };
        newLane.notes = moveNotes(newLane.notes, action.noteId, action.targetId);
        return { ...state, [action.targetLaneId]: newLane };
      }*/
    //  console.log('Czy w to wpadamy? ', action.targetId);
    ...
    case MOVE_BETWEEN_LANES: {
      ...
      // targetLane.notes = moveNotes(targetLane.notes, action.noteId, action.targetId);
